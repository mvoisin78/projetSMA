/**
 * 
 * Paramètres de création du ManagerAgents : 3,abc,bca,20
 * 
 */
package project;
...
public class ManagerAgents extends Agent {
	...
	protected void setup() {
		...
		if (args != null) {
			if (args.length == 4) {
				...
				System.out.println("Des paramètres par défaut ont été mis en absence d'arguments valides");
				printArgs(nbAgents, agentsType, agentsCons, simuDuration);
			}
		}
		...
		if (nbAgents == agentsType.length() && nbAgents == agentsCons.length()) {
			...
			System.out.println("Paramètres non valides, merci de réésayer");
		}
	...
	void startAll(ArrayList<AgentController> agents) {
		System.out.println("Démarage des agents");
	...
	void printArgs(int nbAgents, String agentsType, String agentsCons, int simuDuration) {
		...
		System.out.println("Durée de la simulation : " + simuDuration + " unitées de temps");
	}
